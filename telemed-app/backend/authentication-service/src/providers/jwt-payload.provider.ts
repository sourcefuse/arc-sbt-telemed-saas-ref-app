...
import {User, UserTenant} from '@sourceloop/authentication-service';
...
import {JwtPayloadFn} from '@sourceloop/authentication-service';

export class CustomJwtPayloadProvider implements Provider<JwtPayloadFn> {
  constructor(
  ...
  value() {
    return async (
      ...
      const finalpayload = authUser.toJSON();
