...
export class MySequence implements SequenceHandler {
  ...
  constructor(
    ...
    protected authenticateRequest: AuthenticateFn<AnyObject>,
  ...
  async handle(context: RequestContext) {
    ...
    try {
      ...
      await this.setupDatasource(context as any); // keeping this before the authenticateRequest etc. middleware is important because those talk with db

      await this.authenticateRequestClient(request);

      const authUser: AnyObject = await this.authenticateRequest(
        request,
        response,
      );
