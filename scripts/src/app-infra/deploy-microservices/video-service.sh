...
output=$(npx db-migrate db:create $APP_PARAM_VIDEO_SERVICE_DB_NAME --config ./migrations/database.json 2>&1) || exit_code=$?

# Check if the error message contains "already exists"
if [[ "$output" == *"already exists"* ]]; then
    echo "Database '$APP_PARAM_VIDEO_SERVICE_DB_NAME' already exists, skipping creation."
else
...
DB_DATABASE=$APP_PARAM_VIDEO_SERVICE_DB_NAME npx db-migrate up --config ./migrations/database.json
...
EOF
  ...
  echo "Video Service deployment skipped due to NO_SERVICE_DEPLOY=yes"
fi
