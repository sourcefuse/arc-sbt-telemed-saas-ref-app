...
export DOMAIN_NAME=$APP_PARAM_AUTH_SERVICE_CUSTOM_DOMAIN 
...
output=$(npx db-migrate db:create $APP_PARAM_AUTH_SERVICE_DB_NAME --config ./migrations/database.json 2>&1) || exit_code=$?

# Check if the error message contains "already exists"
if [[ "$output" == *"already exists"* ]]; then
    echo "Database '$APP_PARAM_AUTH_SERVICE_DB_NAME' already exists, skipping creation."
else
...
DB_DATABASE=$APP_PARAM_AUTH_SERVICE_DB_NAME npx db-migrate up --config ./migrations/database.json
...
if [ "$NO_SERVICE_DEPLOY" != "yes" ]; then
    ...
    "DB_DATABASE": "$(echo $APP_PARAM_AUTH_SERVICE_DB_NAME)",
    "DB_SCHEMA": "main",
...
EOF
  ...
  echo "Authentication Service deployment skipped due to NO_SERVICE_DEPLOY=yes"
fi

export SBT_OUTPUT_AUTH_SERVICE_URL=$(aws cloudformation describe-stacks --stack-name $STACK_NAME --query "Stacks[0].Outputs[?OutputKey=='DomainName'].OutputValue" --output text)
